...
const login = async (req: Request, res: Response) => {
  try {
    const userRes: Client | null = await client.get(req.body.email);
...
const register = async (req: Request, res: Response) => {
  try {
    ...
    if (userRes) {
      ...
      let userData;
      if (!userRes) {
        userData = await client.create(req.body);
      }
...
const updaterUserInfo = async (req: Request, res: Response) => {
  try {
    const userInfo = await client.getById(req.params.id);
    if (userInfo) {
      let hash: string;
      ...
      const updateStatus = await client.update(req.body, req.params.id);
      if (updateStatus) {
...
const verifyOtp = async (req: Request, res: Response) => {
  try {
    let userInfo: any = await client.get(req.body.email);
    ...
    if (userInfo && otp && userInfo.expiration_time < isExpired) {
      await client.updateByEmail({ is_active: true }, userInfo.email);
      return responseHelper.successResponse(res, StatusCodes.OK)(
        "User Verified",
        userInfo
      );
...
const resendOtp = async (req: Request, res: Response) => {
  try {
    let userData: any = await client.get(req.body.email);
    if (req.body.email) {
      //Generate OTP
...
const checkPassword = async (req: Request, res: Response) => {
  try {
    const userInfo = await client.get(req.body.email);
