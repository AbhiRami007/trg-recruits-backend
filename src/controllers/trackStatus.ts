...
const addStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (track) {
        return responseHelper.successResponse(res, StatusCodes.OK)("Status added Successfully", track);
    }
  } catch (error) {
...
const getStatus = async (req: Request, res: Response) => {
    try {
      ...
      if (track) {
          return responseHelper.successResponse(res, StatusCodes.OK)("Status fetched Successfully", track);
      }
    } catch (error) {
      responseHelper.errorResponse(res, StatusCodes.INTERNAL_SERVER_ERROR)(error.errors[0].message);
    }
